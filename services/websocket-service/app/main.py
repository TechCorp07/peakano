...
from app.services.presence_manager import init_presence_manager
from app.services.event_handler import init_event_handler
...
async def lifespan(app: FastAPI):
    ...
    redis = get_redis()
    init_presence_manager(redis, ttl=settings.REDIS_PRESENCE_TTL)
    ...
    logger.info(f"WebSocket endpoint: ws://localhost:{settings.PORT}/ws")
...
# Create FastAPI app
app = FastAPI(
    title="Medical Imaging WebSocket Service",
    description="Real-time collaboration for medical imaging annotations",
